...
export default async function AttendanceChart() {
  const attendance = await fetchAttendance();

  const chartHeight = 350;
  console.log("attendance chart function data:", attendance);

  const { yAxisLabels, topLabel } = generateYAxis(attendance);
    ...
    <div className="w-full md:col-span-4">
      ...
      <div className="rounded-xl bg-gray-50 p-4">
        ...
        <div className="flex items-center pb-2 pt-6">
          ...
          <h3 className="ml-2 text-sm text-gray-500 ">Upcoming attendance by day</h3>
        </div>
      </div>
    </div>
  );
